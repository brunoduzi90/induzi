...
/**
 * Obter versao do schema armazenada no banco (tabela configuracoes)
 */
function getDbVersion(PDO $pdo): string {
    try {
        $stmt = $pdo->prepare("SELECT valor FROM configuracoes WHERE chave = 'system_schema_version'");
        $stmt->execute();
        $row = $stmt->fetch();
...
/**
 * Salvar versao do schema no banco (tabela configuracoes)
 */
function setDbVersion(PDO $pdo, string $version): void {
    $stmt = $pdo->prepare("INSERT INTO configuracoes (chave, valor) VALUES ('system_schema_version', ?)
...
function downloadRemoteFile(string $url, string $destPath, int $timeout = 300, string $authToken = ''): array {
    ...
    // Garantir diretorio de destino existe
    ...
    // Fallback: file_get_contents + file_put_contents
    if (ini_get('allow_url_fopen')) {
        $httpHeaders = "User-Agent: $userAgent\r\n";
        if ($authToken) {
            $httpHeaders .= "Authorization: Bearer $authToken\r\n";
