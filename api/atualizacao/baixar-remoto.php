...
$updateToken = getGlobalConfig('INDUZI_UPDATE_TOKEN', '');
...
// browser: https://github.com/OWNER/REPO/releases/download/TAG/FILE
// api:     https://api.github.com/repos/OWNER/REPO/releases/assets/ASSET_ID
if ($updateToken && preg_match('#^https://github\.com/([^/]+)/([^/]+)/releases/download/([^/]+)/(.+)$#', $downloadUrl, $m)) {
    ...
    // Buscar release para encontrar o asset ID
        ...
        if (is_array($releaseData) && !empty($releaseData['assets'])) {
            foreach ($releaseData['assets'] as $asset) {
                if ($asset['name'] === $filename) {
                    $downloadUrl = $asset['url']; // api.github.com URL que aceita auth
                    break;
                }
            }
        }
...
if ($freeSpace !== false && $freeSpace < 104857600) { // 100MB minimo
...
// Detectar se URL aponta para o proprio servidor (evita WAF bloqueando request local)
$localFile = null;
...
if ($serverHost && strcasecmp($urlHost, $serverHost) === 0) {
    // URL local: converter para path de arquivo
    $urlPath = $parsedUrl['path'] ?? '';
    // Obter o path base da aplicacao
...
if ($localFile) {
    // Copiar arquivo local (sem HTTP, sem WAF)
...
$db = getDB();
logActivity($db, null, $session['userId'], 'atualizacao_remota', [
    'de' => $oldVersion,
    'para' => $newVersion,
    'arquivos' => count($updatedFiles),
    'migracoes' => $migrationResult['ok'] ? 'OK' : 'Erro',
]);
...
// Separar arquivos criados vs atualizados e gerar resumo compacto
$filesCreated = [];
$filesUpdatedList = [];
